...
: mc_control::MCController(rm, dt)
{
	...
	int argc = 0;
	ros::init(argc, std::vector<char*>({""}).data(), "RandomGoalController", ros::init_options::NoSigintHandler);
	mc_rtc::log::info("RandomGoalController: ROS {}", ros::ok());
...
void RandomGoalController::switch_target()
{
	...
	while(is_colliding)
	{
		...
		if(client.call(request, response))
		{
			...
			mc_rtc::log::info("RandomGoalController: Collision Check Result: {}, {:4f} {:4f} {:4f} {:4f}", is_colliding, targets[jointNames[0]][0], targets[jointNames[1]][0], targets[jointNames[2]][0], targets[jointNames[3]][0]);
		}
		else
		{
			mc_rtc::log::error("RandomGoalController: Collision Check Failed {} {}", client.isValid(), strcmp(ros::service_traits::md5sum(request), ros::service_traits::md5sum(response)));
		}
	}
